...
<style>
    ...
    .circulo1{
        background: radial-gradient(circle closest-side,orange 0%,orange 98%,rgba(0,0,0,0) 100%) !important;
        width: 20px !important;
        height: 20px!important;
        float: left;
    }
    .circulo2{
        background: radial-gradient(circle closest-side,green 0%,green 98%,rgba(0,0,0,0) 100%) !important;
        width: 20px !important;
        height: 20px!important;
        float: left;
    }
    .circulo3{
        background: radial-gradient(circle closest-side,#00BFFF 0%,#00BFFF 98%,rgba(0,0,0,0) 100%) !important;
        width: 20px !important;
        height: 20px!important;
        float: left;
    }
...
<body>
    ...
    <script>
        ...
        $(document).on('click','.edit',function(){
            ...
            $paciente       = $(this).closest('tr').find('td').eq(getColumnValue("idPaciente")).text();
            $enfermeiro     = $(this).closest('tr').find('td').eq(getColumnValue("idEnfermeiro")).text();
            $medico         = $(this).closest('tr').find('td').eq(getColumnValue("idMédico")).text();
        ...
        $(document).on('click','table tbody tr',function(){
            $("table tbody tr").removeClass('corLinhaTable');
            $(this).addClass('corLinhaTable');
            
        });
        
        $(document).on('click','.circulo1Click',function(){
            
            $idCirurgia     = $(this).closest('tr').find('td').eq(getColumnValue("idCIRURGIA")).text();
            $paciente       = $(this).closest('tr').find('td').eq(getColumnValue("idPaciente")).text();
            $nomePaciente   = $(this).closest('tr').find('td').eq(getColumnValue("Paciente")).text();
            
            setServicos("pre",$idCirurgia, $paciente, $nomePaciente);
        });
        $(document).on('click','.circulo2Click',function(){
            
            $idCirurgia     = $(this).closest('tr').find('td').eq(getColumnValue("idCIRURGIA")).text();
            $paciente       = $(this).closest('tr').find('td').eq(getColumnValue("idPaciente")).text();
            $nomePaciente   = $(this).closest('tr').find('td').eq(getColumnValue("Paciente")).text();
            
            setServicos("trans",$idCirurgia, $paciente, $nomePaciente);
        });
        $(document).on('click','.circulo3Click',function(){
            
            $idCirurgia     = $(this).closest('tr').find('td').eq(getColumnValue("idCIRURGIA")).text();
            $paciente       = $(this).closest('tr').find('td').eq(getColumnValue("idPaciente")).text();
            $nomePaciente   = $(this).closest('tr').find('td').eq(getColumnValue("Paciente")).text();
            
            setServicos("pos",$idCirurgia, $paciente, $nomePaciente);
        });
        
        function setServicos($tipoEx,$idCirurgia, $paciente, $nomePaciente)
        {
            $.ajax({
                url: 'php/ajax/cirurgia.php',
                type: 'POST',
                ...
                success: function() {
                    window.location.href = "add_servico_cirurgico.php";
                },
            });
        }
    </script>
